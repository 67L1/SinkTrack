...
def get_res(prompt, image, one_shot):
    ...
    with torch.inference_mode():
        generation = model.generate(**inputs, max_new_tokens=2048, do_sample=True, injection_layer_idx=injection_layer_idx)
...
def main(SEED):
    ...
    fix_seeds(SEED)
    output_dir = './sintrack/qwen4b/{}'.format(DATA_NAME)
    ...
    for idx, data in enumerate(tqdm(dataset)):
        try:
            ...
            if cot_prompt in zero_shot_mcot_input_str:
                injection_layer_idx = 4
            else:
