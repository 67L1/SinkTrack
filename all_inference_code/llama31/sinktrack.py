    ...
    def forward(
        ...
        if is_injection_step:
            ...
            if global_prompt_embedding.dim() == 2:
                global_prompt_embedding = global_prompt_embedding.unsqueeze(1)
...
class LlamaInjectionDecoderLayer(LlamaDecoderLayer):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        super().__init__(config, layer_idx)
        self.self_attn = LlamaInjectionAttention(config=config, layer_idx=layer_idx)
...
class LlamaModelWithPromptInjection(LlamaModel):
    def __init__(self, config: LlamaConfig):
        super().__init__(config)
        self.layers = nn.ModuleList(
            [LlamaInjectionDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.post_init()
    ...
    def forward(
            ...
            global_prompt_embedding: Optional[torch.FloatTensor] = None,
            injection_layer_idx: Optional[int] = None,
            **kwargs: Unpack[TransformersKwargs],
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if (injection_layer_idx is not None and
...
class LlamaForCausalLMWithPromptInjection(LlamaForCausalLM):
    def __init__(self, config):
        LlamaPreTrainedModel.__init__(self, config)
        self.model = LlamaModelWithPromptInjection(config)
    ...
    def forward(
        ...
        if injection_layer_idx is not None and inputs_embeds.shape[1] > 1 and attention_mask is not None:
            ...
            for i in range(inputs_embeds.shape[0]):
                ...
                valid_embeds = inputs_embeds[i, :int(prompt_length), :]
            ...
            global_prompt_embedding = torch.stack(prompt_embeddings_list, dim=0)
