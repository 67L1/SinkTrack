...
def get_res(prompt, image, one_shot):
    ...
    generated_ids = model.generate(**inputs, max_new_tokens=1024, temperature=1.2, do_sample=True)
...
def main():
    if '3B' in path:
        output_dir = './cot/qwen3b/{}'.format(DATA_NAME)
    else:
        output_dir = './cot/qwen7b/{}'.format(DATA_NAME)
