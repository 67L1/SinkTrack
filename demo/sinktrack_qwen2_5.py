...
class Qwen2InjectionDecoderLayer(Qwen2DecoderLayer):
    ...
    def forward(
            ...
            global_prompt_embedding: Optional[torch.Tensor] = None,  # 新增参数
            injection_layer_idx: Optional[int] = None,  # 新增参数
            **kwargs: Unpack[TransformersKwargs],
    ) -> tuple[torch.Tensor]:
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            ...
            if (
                    ...
                    idx != (self.config.num_hidden_layers - 1)  # 不在最后一层之后更新
            ):
