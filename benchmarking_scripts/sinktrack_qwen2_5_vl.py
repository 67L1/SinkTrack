...
class Qwen2_5_VLInjectionAttention(Qwen2_5_VLAttention):
    """
    继承自 Qwen2_5_VLAttention，增加了在指定层向第一个 token 注入全局图像信息的功能。
...
class Qwen2_5_VLTextModelWithInjection(Qwen2_5_VLTextModel):
    ...
    def forward(
        ...
        # (此处应有完整的 position_ids, cache_position, attention_mask 处理逻辑)
        # 此处仅为演示，实际需要完整复制
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            layer_outputs = decoder_layer(
                hidden_states,
                attention_mask=causal_mask_mapping[decoder_layer.attention_type],  # 简化处理，实际应使用 causal_mask_mapping
                position_ids=text_position_ids,  # 简化处理，实际应使用 text_position_ids
        ...
        if pixel_values is not None:
            ...
            if injection_layer_idx is not None:
                ...
                if global_image_embeddings_list:
                    ...
                    # mean_input_embeds = torch.mean(inputs_embeds, dim=1)
                    # mean_input_embeds = inputs_embeds[:,:1,:]
                    # global_image_embedding_batch = 0.8 * global_image_embedding_batch + mean_input_embeds * 0.2
...
class Qwen2_5_VLForConditionalGenerationWithInjection(Qwen2_5_VLForConditionalGeneration):
    def __init__(self, config):
        # 调用父类的 __init__
        super().__init__(config)
        # 关键: 用我们自定义的 Text Model 替换掉原始模型中的 language_model
