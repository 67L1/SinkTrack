...
def get_response(prompt_text: str, tokenizer, model, max_new_tokens: int, do_sample: bool, injection_layer_idx: int) -> str:
    messages = [
        {
            "role": "system",
...
def run_inference(val_data, tokenizer, model, max_new_tokens, do_sample, injection_layer_idx, output_path):
        ...
        for article in tqdm(val_data, desc="Articles"):
            for paragraph in tqdm(article["paragraphs"], desc="Dialogues", leave=False):
                ...
                for qa in paragraph["qas"]:
                    ...
                    pred_span = get_response(prompt, tokenizer, model, max_new_tokens, do_sample, injection_layer_idx)
...
if __name__ == "__main__":
    ...
    output_stem         = cfg["output"]["sinktrack_output_path"]
    max_new_tokens      = cfg["generation"]["max_new_tokens"]
    do_sample           = cfg["generation"]["do_sample"]
    injection_layer_idx = cfg["sinktrack"]["injection_layer_idx"]
    seeds               = cfg["experiment"]["seeds"]

    tokenizer = AutoTokenizer.from_pretrained(model_path)
    model = LlamaForCausalLMWithPromptInjection.from_pretrained(
        model_path,
    ...
    for seed in seeds:
        ...
        run_inference(val_data, tokenizer, model, max_new_tokens, do_sample, injection_layer_idx, output_path)