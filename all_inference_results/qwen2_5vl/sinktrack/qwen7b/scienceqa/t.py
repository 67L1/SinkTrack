...
letter_map = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L',]
...
def process_single_file(input_file: str) -> Tuple[float, float]:
    ...
    with open(input_file, "r", encoding="utf-8") as f:
        for ln, line in enumerate(f, start=1):
            ...
            question = str(obj.get("prompt", "")).strip()
            pred_extracted = extract_from_pred(pred_raw)
            pred_extracted = pred_extracted.replace('answer', '').strip()
            # if '.' in pred_extracted:
            #     pred_extracted = pred_extracted.split('.')[0]
            ...
            if compare_pred_answer(pred_extracted, ans):
                ...
                options_map = parse_options_from_question(choices)
                if '.' in pred_extracted and len(pred_extracted.split('.')[0].strip()) == 1:
                    pred_extracted = pred_extracted.split('.')[0].strip()
                    if pred_extracted.lower() == ans.lower():
                        ok = True
                ans_key = ans.upper()
            ...
            if ok:
                ...
                if len(errors) < show_errors and ans not in ['a', 'b', 'c', 'd', 'e', 'f',
                                                             'g'] and pred_extracted not in ['a', 'b', 'c', 'd', 'e',
                                                                                             'f', 'g']:
                    errors.append({
                        "line_no": ln, "id": obj.get("idx"), "question": question,
...
if __name__ == "__main__":
    ...
    input_files = [
        ...
        # 'res_d_323.json',
        '/file02/user/liuxu/Interleaved/qwen/scaffold/results/scienceQA/res.json',
        '/file02/user/liuxu/Interleaved/qwen/scaffold/results-direct/scienceQA/res.json',
        '/file02/user/liuxu/Interleaved/qwen/scaffold/results-direct/scienceQA/res_cot.json',

        # '/file02/user/liuxu/Interleaved/qwen/scaffold/results-cot/scienceQA/res.json'
        'res_323.json',
        'res_2_323.json',
    ]
