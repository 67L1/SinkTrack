...
def get_res(prompt, image, one_shot):
    ...
    with torch.inference_mode():
        generation = model.generate(**inputs, max_new_tokens=2048, do_sample=False, injection_layer_idx=injection_layer_idx)
...
def main():
    output_dir = './results-cot-token-wo-mean/qwen/{}'.format(DATA_NAME)
    os.makedirs(output_dir, exist_ok=True)

    mcot_zero_fh = open(output_dir + '/res_wo_pos_cu.json', 'a')
